...
if (SERVER) then
	...
	function PLUGIN.PlayerAllowedMsg( PLAYER, OWNER, THING )
		...
		if (!PLUGIN.PlayerAllowed(PLAYER, OWNER, THING)) then
			...
			ASS_MessagePlayer(PLAYER, "Item is protected!")
			return false
	...
	function PLUGIN.GetOwner( ENTITY )
		...
		if (!ValidEntity(ENTITY)) then return NullEntity() end
		...
		if (ENTITY.Player && type(ENTITY.Player) == "Player" && ValidEntity(ENTITY.Player)) then
		...
		if (ENTITY.GetPlayer && type(ENTITY.GetPlayer) == "function") then
			...
			if (type(P) == "Player" && ValidEntity(P)) then 
	...
	function PLUGIN.CanPlayerEnterVehicle( PLAYER, VEHICLE, ROLE )	if (!PLUGIN.PlayerAllowedMsg(PLAYER, PLUGIN.GetOwner(VEHICLE), ":ride")) then return false end	end	
...
if (CLIENT) then
	...
	function PLUGIN.BuddyAdd(BUDDY)
		...
		if (!ValidEntity(BUDDY)) then return end
	...
	function PLUGIN.PropProtectionNonAdmin(MENU)
		...
		MENU:AddSubMenu("Add Buddy", nil, function(NEWMENU) ASS_PlayerMenu( NEWMENU, {}, PLUGIN.BuddyAdd ) end )
		MENU:AddOption("Remove Buddy...",	PLUGIN.BuddyRemove )
	...
	function PLUGIN.AddNonAdminMenu(MENU)
		...
		MENU:AddSubMenu( "Prop Protection", nil, PLUGIN.PropProtectionNonAdmin )
	...
	function PLUGIN.PropProtection(MENU)
		...
		if (Items[Mode]) then
			Items[Mode]:SetImage("gui/silkicons/check_on_s")
		end
	...
	function PLUGIN.AddGamemodeMenu(DMENU)			
		...
		DMENU:AddSubMenu( "Prop Protection" , nil, PLUGIN.PropProtection )
	...
	function PLUGIN.HUDPaint()
		...
		if (tr.Hit && ValidEntity(tr.Entity)) then
			...
			if (!ValidEntity(OWNER)) then
				...
				elseif (ENTITY.GetPlayer && type(ENTITY.GetPlayer) == "function") then
					...
					if (type(OWNER) == "Player" && ValidEntity(OWNER)) then 
