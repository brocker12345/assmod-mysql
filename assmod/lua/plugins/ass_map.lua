...
if (SERVER) then
	...
	function PLUGIN.PlayerInitialSpawn(PLAYER)
		timer.Simple(1, PLUGIN.RefreshMapList, PLAYER)
	end
...
if (CLIENT) then
	...
	function PLUGIN.MapMenu(MENU)
		...
		if (!maplistLoaded) then
			...
			if currentMap != "" then MENU:AddSpacer(); end
			MENU:AddOption("Loading... " .. math.floor((#allMaps / numToLoad) * 100) .. "%", function() end )
