...
if (CLIENT) then
	...
	function PLUGIN.SpamProtection(MENU)
		MENU:AddSubMenu( "Spawning",	nil, function(NEWMENU) PLUGIN.SpamProtectionTime(NEWMENU, "spawn") end )
		MENU:AddSubMenu( "Tool",	nil, function(NEWMENU) PLUGIN.SpamProtectionTime(NEWMENU, "tool") end )
	end

	function PLUGIN.AddGamemodeMenu(DMENU)			

		DMENU:AddSubMenu( "Spam Protection" , nil, PLUGIN.SpamProtection )
