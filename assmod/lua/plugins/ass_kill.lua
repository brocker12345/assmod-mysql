...
if (SERVER) then
	...
	expl:SetPos(TO_KILL:GetPos())
	expl:SetParent(TO_KILL)
	expl:SetOwner(PLAYER)
	...
	TO_KILL:Kill()
				...
				SendLog(PLAYER:Nick(), "KILL_PLAYER", TO_KILL:Nick(), nil)
...
if (CLIENT) then
	...
	function PLUGIN.AddMenu(DMENU)			
		...
		DMENU:AddSubMenu( "Kill" , nil, PLUGIN.BuildMenu):SetImage( "gui/silkicons/bomb" )
