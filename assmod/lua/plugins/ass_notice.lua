...
if (SERVER) then
	...
	function PLUGIN.AddFixedNotice( PLAYER, CMD, ARGS )
		...
		if (PLAYER:IsAdmin()) then
			...
			SendLog(PLAYER:Nick(), "ADD_NOTICE", nil, msg)
		end
	end
	...
	function PLUGIN.AddNotice( PLAYER, CMD, ARGS )
		if (PLAYER:IsTempAdmin()) then
			...
			SendLog(PLAYER:Nick(), "ADD_NOTICE_TEMP", nil, msg)
		end
	end
	...
	function PLUGIN.RemoveNotice( PLAYER, CMD, ARGS )
		if (PLAYER:IsTempAdmin()) then
			...
			SendLog(PLAYER:Nick(), "REMOVE_NOTICE", nil, name)
...
if (CLIENT) then
	...
	function PLUGIN.NoticeMenu(MENU)			
		...
		MENU:AddOption( "Add Notice...", 	function() PLUGIN.NoticeGUI(false) end )
		MENU:AddOption( "Add Fixed Notice...",	function() PLUGIN.NoticeGUI(true) end )
		MENU:AddOption( "Remove Notice...", 	function() PLUGIN.RemoveNotice() end )

	end

	function PLUGIN.AddMenu(DMENU)			
	
		DMENU:AddSubMenu( "Notices", nil, PLUGIN.NoticeMenu ):SetImage( "gui/silkicons/newspaper" )
